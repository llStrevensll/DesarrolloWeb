...
if(isset($_POST['submit'])):
  ...
  try{
    ...
    $stmt = $conn->prepare("SELECT cat_evento, COUNT(DISTINCT nombre_evento) FROM eventos INNER JOIN categoria_evento ON eventos.id_cat_evento=id_categoria WHERE id_cat_evento=?");
    $stmt->bind_param("s", $id_cat);
    $stmt->execute();
    $stmt->bind_result($categoria_evento, $total);
    $stmt->store_result();//Evitar error en el sql (sincronia en los statement)
    $sql = "INSERT INTO `eventos` (nombre_evento, fecha_evento, hora_evento, id_cat_evento, id_inv, clave) VALUES (?,?,?,?,?,?)";
    $stmt2 = $conn->prepare($sql);
    $stmt->fetch();
    (int) $total = $total;
    $total++;
    $clave = strtolower(substr($categoria_evento, 0, 5)) . "_" . $total;//3 parametros (texto, inicio, finalcorte)
...
<?php include_once 'includes/templates/header.php'; ?>
       ...
       <section class="admin seccion contenedor lol">
         ...
         <form class="invitado" action="agregar_evento.php" method="post">
             ...
             <?php
                try {
                  ...
                  while($cat_eventos = $res->fetch_assoc()){
                    echo '<input type="radio" name="categoria" value='. $cat_eventos['id_categoria'] . '> ' . $cat_eventos['cat_evento']. '</br>';
                  }
           ...
           <div class="campo">
             <label for="invitado">Inivtado:</label>
